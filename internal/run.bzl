...
ERROR_INPUT_PLUGIN_LIST_CONFLICT = "Only one of `plugins` or `plugins_info` can be provided, but not both."
...
def _run_one(ctx, input_css, input_map, output_css, output_map):
    ...
    # Try `plugins` first, then `plugins_info`. Both can't be set.
    plugins = ctx.attr.plugins
    if ctx.attr.plugins_info and ctx.attr.plugins_info[PostcssPluginsInfo]:
        if len(plugins.items()) == 0:
            plugins = ctx.attr.plugins_info[PostcssPluginsInfo].plugins
        else:
            fail(ERROR_INPUT_PLUGIN_LIST_CONFLICT)

    # Make sure there is at least one plugin.
    if plugins == None or len(plugins.items()) == 0:
